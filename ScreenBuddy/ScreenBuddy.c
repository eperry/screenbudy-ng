...
static void Log_Init(void) {
	...
	g_LogFile = _wfopen(LogFilePathW, L"w");
	
	if (g_LogFile) {
		time_t now = time(NULL);
		fprintf(g_LogFile, "================================================================================\n");
		fprintf(g_LogFile, "  ScreenBuddy Log - %s\n", BUDDY_VERSION_STRING);
		fprintf(g_LogFile, "  Log started: %s", ctime(&now));
...
static void Log_Close(void) {
	if (g_LogFile) {
		time_t now = time(NULL);
		fprintf(g_LogFile, "\n================================================================================\n");
		fprintf(g_LogFile, "  Log ended: %s", ctime(&now));
		fprintf(g_LogFile, "================================================================================\n");
		fclose(g_LogFile);
		g_LogFile = NULL;
	}
...
static void Log_Write(LogLevel level, const char* func, int line, const char* fmt, ...) {
	if (!g_LogFile) return;
...
static void Log_WriteW(LogLevel level, const char* func, int line, const wchar_t* fmt, ...) {
	if (!g_LogFile) return;
...
static void Buddy_LoadConfig(ScreenBuddy* Buddy)
{
	...
	// Load window capture preferences
	Buddy->CaptureFullScreen = GetPrivateProfileIntW(BUDDY_CONFIG, L"CaptureFullScreen", 1, Buddy->ConfigPath) != 0;
}
...
static bool Buddy_ShowWindowList(ScreenBuddy* Buddy)
{
	...
	if (!Result.Cancelled)
	{
		...
		if (Result.SelectedWindow == NULL)
		{
			...
			WritePrivateProfileStringW(BUDDY_CONFIG, L"CaptureFullScreen", L"1", Buddy->ConfigPath);
		}
		else
		{
			Buddy->SelectedWindow = Result.SelectedWindow;
			Buddy->CaptureFullScreen = false;
			WritePrivateProfileStringW(BUDDY_CONFIG, L"CaptureFullScreen", L"0", Buddy->ConfigPath);
		}
		return true;
	}
...
static void Buddy_NetworkEvent(ScreenBuddy* Buddy)
{
	...
	while (Buddy->State != BUDDY_STATE_DISCONNECTED)
	{
		...
		if (Recv < 0)
		{
			LOG_ERROR("DerpNet_Recv returned %d - Server disconnected! (TotalSent=%zu, TotalRecv=%zu)", 
				Recv, Buddy->Net.TotalSent, Buddy->Net.TotalReceived);
		...
		{
			{
				...
				{
					{ BUDDY_VERSION_STRING,								0,						BUDDY_DIALOG_LABEL,		0,							BUDDY_DIALOG_NEW_LINE,	},
